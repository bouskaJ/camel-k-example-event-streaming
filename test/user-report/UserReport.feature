Feature: User report and gate-keeper component test
  ...
  Background:
    ...
    And wait for GET on URL http://user-report-system.${YAKS_NAMESPACE}.svc.cluster.local/report/list
    And wait for POST on URL http://gate-keeper.${YAKS_NAMESPACE}.svc.cluster.local
